Shader "ForLT/Step7"
{
	Properties
	{
		...
		_ToneSpeed("ToneSpeed", float) = 0.0
		_ToneMove("ToneMove", float) = 0.0	
	}
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f_img i) : SV_Target
			{
				...
				// 0.0〜1.0 の範囲を 0.0〜2.0 の範囲にした上で、再度 0.0〜1.0 に押し込める
				pos.x = frac(pos.x * 2.0);
				pos.y = frac(pos.y * 2.0);
				...
				float tone = _ToneBase + sin(_Time.x * _ToneSpeed) * _ToneMove;
				c = floor(c * tone) / tone;
